...
async function getConfirmations(txHash) {
  ...
  function confirmEtherTransaction(txHash, uid, amount, confirmations = 10) {
    setTimeout(async () => {
      ...
      if (trxConfirmations >= confirmations) {
        ...
        await db['balances'].increment(['total', 'avail'], {by: amount, where:{uid}})
      ...
      // Recursive call
      return confirmEtherTransaction(txHash, uid, amount, confirmations);
    }, 30 * 1000);
  }

function watchTransfers(to, target, uid){
    ...
    tokenContract.events.Transfer(options, async(err, ev)=>{
        ...
        confirmEtherTransaction(ev.transactionHash, uid, _value);
