...
const XXXget_image_metadata = async (url) => {
  ...
  const metadata = 'any'; // = await sharp(buffer).metadata();    //const metadata = await sharp(response.data).metadata();  //* Takes a little bit more time
...
*/
  let { urllogorounded: urllogo0 } = await findone('forexcurrencies', {
    symbol: base,
  });
  if (urllogo0) {
  } else {
    LOGGER('@!!! base currency invalid');
    return null;
  }
  let { urllogorounded: urllogo1 } = await findone('forexcurrencies', {
    symbol: quote,
  });
  if (urllogo1) {
  } else {
    LOGGER('@!!! quote currency invalid');
    return null;
  }
  ...
  let resp = await joinImages(
    [get_image_path(urllogo0), get_image_path(urllogo1)],
    {
